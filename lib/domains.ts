export interface DomainStatus {
  name: string;
  status: 'pending' | 'reachable' | 'blocked';
}

export interface Service {
  [serviceName: string]: DomainStatus[];
}
...
const rawData = {
	...
	"Social Trackers": {
		...
		"Pinterest": [ "ads.pinterest.com", "log.pinterest.com", "analytics.pinterest.com", "trk.pinterest.com" ],
	...
	"Mix": {
		"Yahoo": [ "ads.yahoo.com", "analytics.yahoo.com", "geo.yahoo.com", "udc.yahoo.com", "udcm.yahoo.com", "analytics.query.yahoo.com", "partnerads.ysm.yahoo.com", "log.fc.yahoo.com", "gemini.yahoo.com", "adtech.yahooinc.com" ],
...
// This function transforms the raw data into the structure we'll use for our state.
export const getInitialState = (): Category => {
  const initialState: Category = {};

  for (const categoryName in rawData) {
    initialState[categoryName] = {};
    const services = (rawData as any)[categoryName];
    for (const serviceName in services) {
      initialState[categoryName][serviceName] = services[serviceName].map((domain: string) => ({
        name: domain,
        status: 'pending'
      }));
    }
